...
interface ServerStore {
  ...
  loadRegularNotifications: () => Promise<RegularNotification[]>;
  addRegularNotification: (
    notification: Omit<RegularNotification, "id" | "createdAt" | "updatedAt">
  ) => Promise<RegularNotification>;
  updateRegularNotification: (
    id: string,
    updates: Partial<RegularNotification>
  ) => Promise<void>;
  deleteRegularNotification: (id: string) => Promise<void>;
  toggleRegularNotification: (id: string) => Promise<void>;
...
export const useServerStore = create<ServerStore>((set, get) => ({
  ...
  updateEmployee: async (
    ...
    try {
      // Пока обновляем локально (можно добавить API вызов при необходимости)
      set((state) => ({
  ...
  // Regular Notifications Actions
  loadRegularNotifications: async (): Promise<RegularNotification[]> => {
    try {
      const response = await apiService.getRegularNotifications();
      const notifications = response.data!;
      set({ regularNotifications: notifications });
      return notifications;
    } catch (error) {
  ...
  addRegularNotification: async (
    notification
  ): Promise<RegularNotification> => {
    try {
      const response = await apiService.createRegularNotification(notification);
      const newNotification = response.data!;
      set((state) => ({
        regularNotifications: [newNotification, ...state.regularNotifications],
      }));
      return newNotification;
    } catch (error) {
      console.error("Failed to add regular notification:", error);
      throw error;
    }
  },

  updateRegularNotification: async (id, updates): Promise<void> => {
    try {
      const response = await apiService.updateRegularNotification(id, updates);
      ...
      console.error("Failed to update regular notification:", error);
      throw error;
    }
  },

  deleteRegularNotification: async (id): Promise<void> => {
    try {
      await apiService.deleteRegularNotification(id);
      set((state) => ({
        regularNotifications: state.regularNotifications.filter(
          (notif) => notif.id !== id
        ),
      }));
    } catch (error) {
      console.error("Failed to delete regular notification:", error);
      throw error;
    }
  },

  toggleRegularNotification: async (id): Promise<void> => {
    try {
      const response = await apiService.toggleRegularNotification(id);
      const updatedNotification = response.data!;
      set((state) => ({
        regularNotifications: state.regularNotifications.map((notif) =>
          notif.id === id ? updatedNotification : notif
      ...
      console.error("Failed to toggle regular notification:", error);
      throw error;
    }
  },
  ...
  getActiveRegularNotifications: (): RegularNotification[] => {
    const state = get();
    return state.regularNotifications.filter((notif) => notif.enabled);
  },
}));
...
interface LocalStore {
  ...
  updateRegularNotification: (
    id: string,
    updates: Partial<
      Omit<RegularNotification, "id" | "createdAt" | "updatedAt">
    >
  ) => void;
...
export const useLocalStore = create<LocalStore>()(
  persist(
    (set, get) => ({
      ...
      addRegularNotification: (notification) => {
        const newNotification: RegularNotification = {
          ...notification,
          id: Date.now().toString(),
        ...
        set((state) => ({
          regularNotifications: [
            ...state.regularNotifications,
            newNotification,
          ],
        }));
      },

      updateRegularNotification: (id, updates) =>
        set((state) => ({
          regularNotifications: state.regularNotifications.map((notif) =>
            notif.id === id
              ? {
...
export const useStore = () => {
  ...
  return {
    ...
    regularNotifications: serverStore.regularNotifications,
    addRegularNotification: serverStore.addRegularNotification,
    updateRegularNotification: serverStore.updateRegularNotification,
    deleteRegularNotification: serverStore.deleteRegularNotification,
    toggleRegularNotification: serverStore.toggleRegularNotification,
    getActiveRegularNotifications: serverStore.getActiveRegularNotifications,
    loadRegularNotifications: serverStore.loadRegularNotifications,
